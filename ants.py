...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            if place.ant.can_contain(self) == True and self.can_contain(place.ant) == False:
                place.ant.store_ant(self)
            elif place.ant.can_contain(self) == False and self.can_contain(place.ant) == True:
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        if not self.double_damaged:
            self.damage *= 2
            self.double_damaged = True
        # END Problem 12
    ...
    def action(self, gamestate):
        ...
        if self.chew_timer == 0:
            if len(current_place.bees) > 0:  
                dead_bee = random.choice(current_place.bees)
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if not gamestate.first_queen:
            gamestate.first_queen = True
    ...
    def action(self, gamestate):
        ...
        while 1: 
            ...
            if current_place.ant:
                ...
                if current_place.ant.is_container == True and current_place.ant.ant_contained:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.is_scared:
            if not (self.is_slowed and gamestate.time % 2 != 0):
                ...
                if not destination.is_hive:
                    if self.blocked():
                        ...
                        self.back_away_turns -= 1
                        if self.back_away_turns == 0:
                            self.is_scared = False
        elif not self.is_slowed or gamestate.time % 2 == 0: 
            if self.blocked():
                self.sting(self.place.ant)
            elif self.health > 0 and destination is not None:
                self.move_to(destination)
